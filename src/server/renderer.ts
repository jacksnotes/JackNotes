...
export default class Renderer extends Model {
    ...
    const customTemplates = files
      ...
      .filter((name: string) => {
        return ![
          ...
          'tags.ejs',
          'archives.ejs',
          // 👇 JackNotes protected word, because these filename is jacknotes folder's name
          'images.ejs',
          'media.ejs',
          'post-images.ejs',
          'styles.ejs',
  ...
  async buildCss() {
    ...
    return new Promise((resolve, reject) => {
      less.render(lessString, { filename: lessFilePath }, async (err: any, output: Less.RenderOutput | undefined) => {
        if (err) {
          console.log(err)
          reject(err)
        }
        if (!output) {
          reject(new Error('Less render output is undefined'))
          return
        }
        let { css } = output
