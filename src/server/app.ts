...
export default class App {
  ...
  public async loadSite() {
    ...
    return {
      ...
      currentThemeConfig,
      appDir: this.appDir,
      mainWindow: null, // Cannot serialize BrowserWindow
    }
  }
  ...
  private async checkDir() {
    ...
    const appConfigFolder = path.join(this.app.getPath('home'), '.jacknotes')
    const appConfigPath = path.join(appConfigFolder, 'config.json')
    let defaultAppDir = path.join(this.app.getPath('documents'), 'JackNotes')
    defaultAppDir = defaultAppDir.replace(/\\/g, '/')

    try {
      // if exist `.hve-notes` config folder, change folder name to `.jacknotes`
      try {
