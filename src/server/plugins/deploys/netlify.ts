...
export default class NetlifyApi extends Model {
  ...
  async request(method: 'GET' | 'PUT' | 'POST', endpoint: string, data?: any) {
    ...
    // Netlify 默认不使用代理，除非明确设置为 netlify 平台且配置了代理
    const proxy = (setting.enabledProxy === 'proxy' && setting.platform === 'netlify') ? {
    ...
    console.log('Netlify request debug:', {
      ...
      enabledProxy: setting.enabledProxy,
      proxy: proxy ? 'using proxy' : 'no proxy',
    })
  ...
  async remoteDetect() {
    try {
      console.log('Netlify remoteDetect: checking site', this.siteId)
      const res = await this.request('GET', 'sites/:site_id/')
      console.log('Netlify remoteDetect response:', res.status, res.data)
      if (res.status === 200) {
        return {
          success: true,
          message: res.data,
      ...
      console.error('Netlify remoteDetect error:', e)
      return {
        success: false,
  ...
  async publish() {
    ...
    try {
      ...
      result.message = `[Server] 同步失败： ${(e && (e as any).message) ? (e as any).message : String(e)}`
      return result
    }
  }
