      ...
      <draggable v-model="menuList" handle=".handle" @change="handleMenuSort">
        ...
        >
          ...
          <div class="p-4 flex-1">
            <div class="text-base text-gray-900 font-medium mb-2">
              {{ menu.name }}
            </div>
            <div class="text-xs flex items-center">
              <div class="text-xs flex items-center px-2 rounded border bg-gray-100 border-gray-200 text-gray-600 mr-4">
                {{ menu.openType }}
...
export default class Menu extends Vue {
  ...
  saveMenu() {
    ...
    ipcRenderer.once('menu-saved', (event: IpcRendererEvent, result: any) => {
      ...
      this.$message.success(this.$t('menuSuccess') as string)
      this.visible = false
    ...
    this.$confirm({
      ...
      onOk: () => {
        ...
        ipcRenderer.once('menu-deleted', (event: IpcRendererEvent, result: any) => {
          ...
          this.$bus.$emit('site-reload')
          this.$message.success(this.$t('menuDelete') as string)
          this.visible = false
        })
      },
    })
