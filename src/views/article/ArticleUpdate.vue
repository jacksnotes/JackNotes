    ...
    <div class="page-content">
      <div class="editor-wrapper">
        ...
        <div class="footer-info">
          {{ $t('writingIn') }} <a @click.prevent="openPage('https://jacknotes.dev')" class="link">JackNotes</a>
        </div>
...
export default class ArticleUpdate extends Vue {
  ...
  formatForm(published?: boolean) {
    ...
    if (!valid) {
      this.$message.error(this.$t('postUrlRepeatTip') as string)
      return
    }

    if (this.form.fileName.includes('/')) {
      this.$message.error(this.$t('postUrlIncludeTip') as string)
      return
    }
