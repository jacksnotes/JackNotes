  ...
  <a-layout>
    ...
    <a-modal title="🔥 New Version" :visible="updateModalVisible" :footer="null" @cancel="updateModalVisible = false" :maskClosable="false">
      <div class="download-container">
        👉 <a href="https://jacknotes.dev">JackNotes Homepage</a> | <a href="https://github.com/jacksnotes/JackNotes/releases">Github Releases</a> 👈
      </div>
      <h2>{{ newVersion }}</h2>
    ...
    <!-- Preview is now a native modal BrowserWindow opened by the main process -->
...
export default class App extends Vue {
  ...
  created() {
    ...
    this.reloadSite() // Initial load site data
    this.checkUpdate()
        ...
        (portGotEvent: IpcRendererEvent, port: number | string | null) => {
          if (!port && typeof port !== 'number') {
            ...
            if (typeof port === 'number' || (typeof port === 'string' && port)) {
              ...
              try {
                ipcRenderer.send('app-preview-open', url)
              } catch (e) {
  ...
  closePreviewModal() {
    // kept for compatibility but no-op since preview is native BrowserWindow now
    try { ipcRenderer.send('app-preview-close') } catch (e) { /* ignore */ }
  }
  ...
  handleGithubClick() {
    ...
    this.openInBrowser('https://github.com/jacksnotes/JackNotes')
  }

  public async checkUpdate() {
    const res = await axios.get('https://api.github.com/repos/jacksnotes/JackNotes/releases/latest')
    if (res.status === 200) {
